...
class DeepNameState(
    collections.namedtuple("DeepNameState", ("cell_state", "last_ids", "prob_c"))):
    ...
    def clone(self, **kwargs):
        ...
        return nest.map_structure(
            with_same_shape,
            self,
            super(DeepNameState, self)._replace(**kwargs))

class DeepName(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self, cell, encoder_states, encoder_input_ids, vocab_size,
        ...
        Args:
            ...
            initial_cell_state:
        """
        super(DeepName, self).__init__(name=name)
        self._cell = cell
        ...
        self._non_copy_weight = tf.get_variable('NonCopyWeight', [self._encoder_state_size, self._cell.output_size])
        self._projection = tf.layers.Dense(self._gen_vocab_size, use_bias=False, name="OutputProjection")

    def __call__(self, inputs, state, scope=None):
        if not isinstance(state, DeepNameState):
            raise TypeError("Expected state to be instance of DeepNameState. "
        ...
        state = DeepNameState(cell_state=cell_state, last_ids=last_ids, prob_c=prob_c)
    ...
    def state_size(self):
        """size(s) of state(s) used by this cell.
            ...
            or TensorShapes.
        """
        return DeepNameState(cell_state=self._cell.state_size, last_ids=tf.TensorShape([]),
    ...
    def zero_state(self, batch_size, dtype):
        with tf.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
            ...
            return DeepNameState(cell_state=cell_state, last_ids=last_ids, prob_c=prob_c)